...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    try {
      ...
      setState(() {
        _userName = doc.exists
            ? (doc.data()?['name'] ?? user.displayName ?? 'Trader')
            : (user.displayName ?? 'Trader');
        _userRole = doc.exists
            ? ((doc.data()?['role'] ?? 'buyer') as String)
            : 'buyer';
        _email  = user.email ?? '';
        _loaded = true;
      });
  ...
  void _fallbackLoad() {
    ...
    final user = FirebaseAuth.instance.currentUser;
    setState(() {
      _userName = user?.displayName ?? 'Trader';
      _userRole = 'buyer';
      _email    = user?.email ?? '';
      _loaded   = true;
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        fit: StackFit.expand,
        children: [
          // Zero-data canvas art background
...
class _BottomNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final items = [
      (Icons.home_rounded,            l.t('home')),
      (Icons.storefront_rounded,      l.t('marketplace')),
      (Icons.chat_bubble_rounded,     l.t('chat')),
      (Icons.directions_boat_rounded, l.t('tracking')),
      (Icons.person_rounded,          l.t('profile')),
    ];
...
class _HomeTab extends StatelessWidget {
  ...
  // ── VOICE: speak dashboard summary ──────────────────────────
  void _speakSummary(BuildContext context) {
    ...
    final greeting = hour < 12 ? 'Good morning'
                   ...
                   : 'Good evening';
    final roleText = userRole == 'seller'  ? 'seller'
                   : userRole == 'shipper' ? 'shipper'
                   : userRole == 'agent'   ? 'agent'
                   : 'buyer';
    final text = '$greeting, $userName. You are logged in as a $roleText. '
        'Welcome to Marine Trade Connect.';
    final langCode = lang.currentCode == 'hi' ? 'hi-IN'
        ...
        : 'en-US';
    voice.speak(text, langCode);
  }

  @override
  Widget build(BuildContext context) {
    final t         = Provider.of<ThemeProvider>(context).current;
    final voice     = Provider.of<VoiceProvider>(context); // ── VOICE
    final uid       = FirebaseAuth.instance.currentUser?.uid ?? '';
    final isSeller  = userRole == 'seller';
    final isShipper = userRole == 'shipper' || userRole == 'agent';
    final hour      = DateTime.now().hour;
    final l = AppLocalizations.of(context);
    final greeting  = hour < 12 ? l.t('good_morning')
                    : hour < 17 ? l.t('good_afternoon')
                    : l.t('good_evening');
        ...
        children: [
                ...
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: Stack(children: [
                      // Asset image hero - zero network data
                      Positioned.fill(
                        child: Image.asset(
                          'assets/images/hero_ocean.jpg',
                          fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) =>
                              CustomPaint(painter: _HeroPainter(t)),
                        ),
                      ),
          ...
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(AppLocalizations.of(context).t('quick_actions'),
                    style: const TextStyle(
                        color: Colors.white,
                        fontSize: 15,
                        fontWeight: FontWeight.w700)),
                ...
                Row(children: [
                  _action(
                    isSeller ? AppLocalizations.of(context).t('post_listing') : AppLocalizations.of(context).t('browse'),
                    isSeller
                ...
                Row(children: [
                  _action(
                      AppLocalizations.of(context).t('messages'),
                      Icons.chat_bubble_rounded,
                      const Color(0xFFAB87FF),
                      () => onTabChange(2),
                      t),
                  const SizedBox(width: 10),
                  _action(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(AppLocalizations.of(context).t('secondhand'),
                                style: const TextStyle(
                                    color: Colors.white,
                                    fontSize: 13,
                                    fontWeight: FontWeight.w700)),
                            Text(
                              'Trade used goods — give items a second life',
                              style: TextStyle(
                                  color:
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text(AppLocalizations.of(context).t('recent_activity'),
                        style: const TextStyle(
                            color: Colors.white,
                            fontSize: 15,
                            fontWeight: FontWeight.w700)),
        ...
        Padding(
          ...
          child: Column(children: [
            _section('Personalisation', [
              _tile(Icons.palette_rounded, AppLocalizations.of(context).t('app_theme'),
                  'Choose your colour scheme', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
                          builder: (_) => const ThemePickerScreen()))),
              _tile(Icons.language_rounded, AppLocalizations.of(context).t('language'),
                  '${currentLang['flag']} ${currentLang['name']}', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
                          builder: (_) => const LanguageScreen()))),
              _tile(Icons.person_rounded, AppLocalizations.of(context).t('edit_profile'),
                  'Update your info', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
            ...
            _section('Preferences', [
              _tile(Icons.notifications_rounded, AppLocalizations.of(context).t('notifications'),
                  'Manage alerts', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
                          builder: (_) => const NotificationsScreen()))),
              _tile(Icons.shield_rounded, AppLocalizations.of(context).t('privacy'),
                  'Control your data', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
            ...
            _section('Support', [
              _tile(Icons.help_rounded, AppLocalizations.of(context).t('help'),
                  'FAQs and contact', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
                          builder: (_) => const HelpSupportScreen()))),
              _tile(Icons.description_rounded, AppLocalizations.of(context).t('terms'),
                  'Legal info', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
                          builder: (_) => const TermsScreen()))),
              _tile(Icons.info_rounded, AppLocalizations.of(context).t('about'), 'Version info', t,
                  () => Navigator.push(context,
                      MaterialPageRoute(
            ...
            GestureDetector(
              onTap: () async {
                // Clear local cache so next login starts fresh
                final user = FirebaseAuth.instance.currentUser;
                if (user != null) {
                  try {
                    final prefs = await SharedPreferences.getInstance();
                    await prefs.remove('user_role_\${user.uid}');
                    await prefs.remove('profile_complete_\${user.uid}');
                    await prefs.remove('lang');
                  } catch (_) {}
                }
                await FirebaseAuth.instance.signOut();
                // authStateChanges() in _AuthGate will redirect to LoginScreen
              },
              child: Container(
                width: double.infinity,
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(AppLocalizations.of(context).t('sign_out'),
                        style: const TextStyle(
...
// ── BACKGROUND PAINTER (zero data, pure canvas) ───────────────────────────────
class _BgPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final glow2 = Paint()
      ..shader = RadialGradient(
        colors: [t.primary.withOpacity(0.05), Colors.transparent],
      ).createShader(Rect.fromCircle(
          center: Offset(0, size.height), radius: size.height * 0.5));
    canvas.drawRect(
        Rect.fromLTWH(0, 0, size.width, size.height), glow2);
...
// ── HERO BANNER PAINTER (zero data, rich feel) ────────────────────────────────
class _HeroPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final orb = Paint()
      ..shader = RadialGradient(
        colors: [t.primary.withOpacity(0.22), Colors.transparent],
      ).createShader(Rect.fromCircle(
          center: Offset(size.width * 0.85, size.height * 0.2),
          radius: size.height));
    canvas.drawRect(Rect.fromLTWH(0, 0, size.width, size.height), orb);

    // Horizontal scan lines — techy feel
    ...
    canvas.drawArc(
      ...
      3.4, 1.0, false, arcPaint,
    );
    canvas.drawArc(
      Rect.fromCircle(
          center: Offset(size.width + 10, size.height + 10),
          radius: size.height * 0.75),
      3.3, 1.1, false,
      arcPaint..color = t.primary.withOpacity(0.07),
    );
  }

  @override
  bool shouldRepaint(_HeroPainter old) => old.t.id != t.id;
}